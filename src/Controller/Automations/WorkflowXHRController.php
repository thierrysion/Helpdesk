...
namespace App\Controller\Automations;
...
class WorkflowXHRController extends AbstractController
{
    private $userService;
    private $translator;
    ...
    public function __construct(UserService $userService, WorkflowListener $workflowListnerService, TicketService $ticketService, FetchDataService $fetchdataService, TranslatorInterface $translator)
    {
    ...
    public function getWorkflowConditionOptionsXHR($entity, Request $request)
    {
        $error = false;
        $json = $results = array();
        $supportedConditions = ['TicketPriority', 'TicketType', 'TicketStatus', 'Zone', 'source', 'agent', 'group','team', 'agent_name', 'agent_email', 'stage'];
        ...
        switch ($entity) {
            ...
            case 'TicketPriority':
                ...
                break;
            case 'Zone':
                $json = $this->fetchdataService->getRegions();
                break;
            default:
                $json = [];
                break;
