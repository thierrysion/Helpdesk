...
class TicketController extends AbstractController
{
    ...
    public function __construct(UserService $userService, UVDeskService $uvdeskService,EventDispatcherInterface $eventDispatcher, TranslatorInterface $translator, TicketService $ticketService, FetchDataService $fetchdataService, ReCaptchaService $recaptchaService, KernelInterface $kernel)
    {
                ...
                if($_POST) {
                    ...
                    if ($form->isValid() && !count($formErrors) && !$error) {
                        ...
                        if(!is_object($data['customer'] = $this->container->get('security.token_storage')->getToken()->getUser()) == "anon.") {
                            ...
                            $data['fullname'] = $name;
                            $userInfos = $em->getRepository(UserInfos::class)->findOneBy(['user' => $userDetails]);
                        }
